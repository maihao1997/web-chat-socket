...
// ma hoa tin nhan
    ...
    try {  
      var publicKey = fs.readFileSync('publicKey.txt', 'utf8');
    } catch(e) {
      console.log('Error:', e.stack);
    }
    
    message = await rsa.encrypted(message, publicKey)


    // message = message.encryptString;
    const createEntry = () => ({ message })
    console.log(message,'mess in handler.js')
